...
public class Persons {
	...
	public void displayCustomer(Statement statement, int proj_num) throws SQLException {
		...
		ResultSet results1 = statement.executeQuery("SELECT customerName, customerTelNum, customerPhyAdd, customerEmail FROM Project_Person_Details WHERE Project_Num = " + proj_num
				+ " AND Person_Type = 'Customer'");
		
		while (results1.next()) {  // Customer details displayed using iterator in table.
			System.out.println( 
					"\nCustomer Name: " + results1.getString("customerName")
					+ "\nContact Number: " + results1.getInt("customerTelNum")
					+ "\nPhysical Address: " + results1.getString("customerPhyAdd")
					+ "\nEmail Address: " + results1.getString("customerEmail")
					);
