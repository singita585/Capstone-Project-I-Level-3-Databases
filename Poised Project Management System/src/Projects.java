...
public class Projects extends PoisedInputChecks {
	...
	public void createProject(Statement statement) throws SQLException {
		...
		int proNumber = intCheck("new project number");
		
		System.out.println("\nPlease add a new project name: ");
		String proName = stringCheck("new project name: "); 
		
		System.out.println("\nPlease add a building type: ");
		String proType = stringCheck("building type");
		
		System.out.println("\nPlease add a address for the project: ");
		String proPhysAdd = stringCheck("project address");
		
		System.out.println("\nPlease add an ERF number: ");
		String proERF = stringCheck("ERF number");
		
		System.out.println("\nPlease add a total fee for the project: ");
		float proTotFee = floatCheck("total fee");
		
		System.out.println("\nPlease add the current amount paid for the project: ");
		float proTotPaid = floatCheck("amount paid");
		
		System.out.println("Please add a deadline for the project (e.g. 3-Dec-2020): ");
		String proDeadline = stringCheck("deadline");
		...
		statement.executeUpdate(
                "INSERT INTO main_project_info VALUES (" + proNumber + ", " + "'" + proName + "'" + ", " + "'" 
        + proType + "'" + ", " + "'" + proPhysAdd + "'" + ", " + "'" + proERF + "'" + ", " + proTotFee + ", " + proTotPaid + ", " + 
        "'" + proDeadline + "'" + ", " + "'" + finalise + "'" + ", " + "'" + comp_date + "'" + ");"
            );
	...
	public void finaliseProject(Statement statement) throws SQLException {
		...
		int proNumber = intCheck("project number");
		
		// Selecting the Total_Fee and Amount_Paid columns from the table.
		ResultSet results2 = statement.executeQuery("SELECT Total_Fee, Amount_Paid FROM main_project_info WHERE Project_Num = " + proNumber);
		float proTotFee = 0;
		float proTotPaid = 0;
		
		// Iterating through the columns and storing the two float numbers into corresponding variables.
		while (results2.next()) {
			proTotFee = results2.getFloat("Total_Fee"); 
			proTotPaid = results2.getFloat("Amount_Paid");
		...
		if (proTotFee == proTotPaid) {
			...
			statement.executeUpdate(
					"UPDATE main_project_info SET Completion_Date = " + "'" + comp_date + "'" + " WHERE Project_Num = " + proNumber
					);
			
			// The project is then marked as finalised by writing 'Yes' to the finalise column in the table.
			statement.executeUpdate(
            		 "UPDATE main_project_info SET Finalised = 'Yes' WHERE Project_Num = " + proNumber
	                );
		 ...
		 */
			...
			customer.displayCustomer(statement, proNumber);
			
			// Added to the customer info, is the amount owing on the project.
			System.out.println("\nAmount Outstanding: R" + (proTotFee - proTotPaid));
	...
	public void findProject(Statement statement) throws SQLException {
		...
		if (search_Choice == 1) {
			...
			ResultSet results6 = statement.executeQuery("SELECT * from main_project_info WHERE Project_Num = " + proNumber);
		 ...
		 */
			...
			String proName = stringCheck("project name");
			
			System.out.println("\nPlease view your project details below: \n");
			
			ResultSet results7 = statement.executeQuery("SELECT * from main_project_info WHERE Project_Name = '" + proName + "'");
